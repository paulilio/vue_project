<template>
  <div v-if="!loading">
    <titulo
      ...
      :btnVoltar="!visualizando.toString()"
    >
...
export default {
  ...
  methods: {
    carregarProfessor() {
      this.$http
        .get(process.env.ROOT_API + "/professor")
        .then(res => res.json())
        .then(professor => {
    ...
    carregarAluno() {
      this.$http
        .get(process.env.ROOT_API + `/aluno/${this.idAluno}`)
        .then(res => res.json())
        .then(aluno => {
          this.aluno = aluno;
          this.loading = false;
    ...
    salvar(_aluno) {
      ...
      this.$http
        .put(process.env.ROOT_API + `/aluno/${_alunoEditar.id}`, _alunoEditar)
        .then(res => res.json())

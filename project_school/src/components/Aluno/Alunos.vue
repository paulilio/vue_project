...
export default {
  ...
  created() {
    if (this.professorid) {
      ...
      this.$http
        .get(`http://localhost:3000/aluno/ByProfessor/${this.professorid}`)
        .then(res => res.json())
  ...
  methods: {
    addAluno() {
      ...
      this.$http
        .post("http://localhost:3000/aluno", _aluno)
        .then(res => res.json())
        .then(aluno => {
    ...
    remover(aluno) {
      this.$http.delete(`http://localhost:3000/aluno/${aluno.id}`).then(() => {
